'''
This version creates an explicit time series for each sample.
...
def build_baseline_ds(issuance: pd.DataFrame, best_track: pd.DataFrame, verbose:bool=True, past_track_limit:int=24):
    ...
    Columns and data types of the combined set:
        ...
        7 : float LATxx, TC position at this point
        8 : float LONxx
        9 : int VMAXxx, TC intensity at this point
    ...
    for i in range(0, past_track_limit+6, 6):
        ...
        columns.append('LAT{0:02d}'.format(i))
        columns.append('LON{0:02d}'.format(i))
...
def main():
    ...
    # write to file
    output_pandas_to_file(dataset, "baseline_dataset_ts_{0}.gz".format(past_track_limit))
